defmodule ApiWeb.StopEventControllerTest do
    ...
    test "can filter by multiple parameters simultaneously", %{conn: conn} do
      ...
      for {filters, expected_ids} <- [
            ...
            {%{"route" => "route1", "direction_id" => "0"},
             ["trip1-route1-v1-1", "trip1-route1-v1-2", "trip2-route1-v2-1", "trip3-route1-v3-2"]},
            # trip + stop
        ...
        assert response_ids == Enum.sort(expected_ids), "Failed for filters: #{inspect(filters)}"
      end
    end
  ...
  describe "show_data/2" do
    ...
    test "can include related resources", %{conn: conn} do
      ...
      conn = get(conn, stop_event_path(conn, :show, @stop_event1.id, %{"include" => "trip,stop"}))
  ...
  describe "invalid parameters" do
    ...
    test "empty or whitespace filter values return empty results", %{conn: conn} do
      ...
      for filter_value <- ["", "   "] do
        conn = get(conn, stop_event_path(conn, :index, %{"filter" => %{"trip" => filter_value}}))
        response = json_response(conn, 200)
        assert response["data"] == []
      end
    end
