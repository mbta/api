...
> [!WARNING]
> Your app will fail when calling `mix phx.server` if `STOP_EVENTS_ENABLED` or `CR_CROWDING_ENABLED` are `true` unless:
>
...
>
> To circumvent these issues, set `STOP_EVENTS_ENABLED` and `CR_CROWDING_ENABLED` to `false`.
